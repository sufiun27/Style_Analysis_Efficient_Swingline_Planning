...
<div class="card">
    <div class="card-body">
        <div class="card-title">
            <h3>{{$length}}X{{$length}} Analysis </h3>
        ...
        <div class="table-responsive">
            <table class="table">
                <tr>
                    <th class="table-dark text-center">Reference/Compare</th>
                    @foreach ($compare as $com)
                    <th class="bg-warning text-center"> <b>{{ $com }}</b> </th>
                    @endforeach
                </tr>
                ...
                @foreach ($reference as $ref)
                  @php $r = $ref; @endphp
                    <tr>
                        <th class="table-info text-center">{{ $ref }}</th>
                        @foreach ($compare as $com)
                            @php
                             
                                $c = $com;
                                $percentage = analysis($r, $c);
                            @endphp
                                ...
                                <form action="{{route('style.compare')}}" method="post">
                                    ...
                                    <input hidden readonly type="text" name="compare" value="{{$c}}">
                                    
                                       <td @if($percentage== 100 )style="background-color:rgba(0, 200, 0,{{$percentage/100}})" @elseif($percentage< 50 )style="background-color:rgba(250, 100, 94,{{1-$percentage/100}}) " @else style="background-color:rgba(94, 241, 94,{{$percentage/100}}) " @endif >
