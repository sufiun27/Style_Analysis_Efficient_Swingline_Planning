        ...
        <div class="table-responsive">
            <table class="table">
                <tr>
                    ...
                    // Create an array to store percentages for sorting
                    $percentages = [];

                    for ($i = 1; $i < $arrayLength; $i++) {
                        $percentage = analysis($r, $styles[$i]);
                        $percentages[$styles[$i]] = $percentage;
                    }

                    // Sort percentages in descending order
                    arsort($percentages);

                    foreach ($percentages as $style => $percentage) {
                    ?>
                        <form action="{{route('style.compare')}}" method="post">
                            @csrf
                            <input hidden readonly type="text" name="referance" value="{{$r}}">
                            <input hidden readonly type="text" name="compare" value="{{$style}}">
